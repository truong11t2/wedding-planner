...
export default function Timeline({ initialWeddingDate }: TimelineProps) {
  ...
  }, [initialWeddingDate, weddingDate, setWeddingDate]);

  // Update temp date when context wedding date changes
  useEffect(() => {
    if (weddingDate && weddingDate !== tempWeddingDate) {
      setTempWeddingDate(weddingDate);
    }
  }, [weddingDate]);
  ...
  const FloatingSaveButton = () => {
    const currentWeddingDate = weddingDate || initialWeddingDate;
    if (!currentWeddingDate) return null;
  ...
  // Use weddingDate from context or prop
  const currentWeddingDate = weddingDate || initialWeddingDate;
